...
export default function Footer() {
    return (
        <motion.footer
            className="bg-blue-900 text-white text-center py-6 mt-20"
            initial={{ opacity: 0, y: 40 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 2.3 }}
        >
            <p className="text-sm">
                © {new Date().getFullYear()} Pemerintah Kabupaten Teluk Bintuni — Satu Peta
            </p>
            <p className="text-xs text-gray-300 mt-1">
                Dikelola oleh Dinas Komunikasi dan Informatika Kabupaten Teluk Bintuni
            </p>
        </motion.footer>
    );
}